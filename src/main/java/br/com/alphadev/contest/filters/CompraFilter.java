...
public class CompraFilter<T> extends BasicFilter {
    ...
    private void adicionarFrom() {
        if (classeGenerica.isInstance(new Compra())) {
            filterDescription.append("SELECT c FROM br.com.alphadev.contest.entity.Compra c ");
            filterDescription.append("WHERE 1 = 1 ");
        }
        else if (classeGenerica.isInstance(new Pagar())) {
            filterDescription.append("SELECT p FROM br.com.alphadev.contest.entity.Pagar p ");
            ...
            filterDescription.append("   FROM br.com.alphadev.contest.entity.Pago pg1 ");
            ...
            filterDescription.append("   FROM br.com.alphadev.contest.entity.Pagar p2 ");
            filterDescription.append("   WHERE NOT EXISTS ELEMENTS(p2.pagamentos) ");
            filterDescription.append(") ");*/
        }
        else if (classeGenerica.isInstance(new Pago())) {
            filterDescription.append("SELECT g FROM br.com.alphadev.contest.entity.Pago g ");
            filterDescription.append("WHERE 1 = 1 ");
        }
    }
    ...
    private void adicionarVencimento() {
        if (classeGenerica.isInstance(new Compra())) {
            if (dataVencimentoInicial != null) {
                ...
                filterDescription.append("   SELECT p.compra.id FROM br.com.alphadev.contest.entity.Pagar p ");
